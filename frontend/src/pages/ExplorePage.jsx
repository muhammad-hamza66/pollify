    ...
    <div className="space-y-6 animate-fade-up">
      <div>
        ...
        <p className="text-sm text-[#64748b] dark:text-gray-400 mt-0.5">
          Discover what the community is voting on right now.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="w-px h-5 bg-[#e2e8f0] dark:bg-gray-700 mx-1" />
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={
            feed === "following"
              ? "Follow more creators or switch back to all polls."
              : "Try a different poll type."
          }
        />
      ) : (
...
function FilterChip({ active, onClick, children }) {
  return (
    <button
      onClick={onClick}
      className={clsx(
        "flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-sm font-medium transition-all duration-150 whitespace-nowrap",
        active
          ? "bg-primary-600 text-white shadow-sm shadow-primary-600/20"
          : "bg-gray-100 text-[#64748b] dark:bg-gray-800 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700"
      )}
    >
      {children}
    </button>
