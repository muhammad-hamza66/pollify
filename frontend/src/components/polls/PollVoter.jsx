    ...
    return (
      ...
      >
        ...
        <button
          ...
          className="btn bg-primary-600 text-white hover:bg-primary-700 px-4 disabled:opacity-40 shadow-sm shadow-primary-600/20"
        >
          Send
        </button>
      </form>
  ...
  if (!showResults) {
    return (
      <button
        ...
        disabled={disabled}
        className={clsx(
          "w-full flex items-center gap-3 rounded-xl border text-left transition-all duration-150",
          "border-[#e2e8f0] dark:border-gray-700 hover:border-primary-400 hover:bg-primary-50/40 dark:hover:bg-primary-500/5",
      ...
      >
        ...
        <span className="flex-1 font-medium text-[#0f172a] dark:text-gray-200">
          {option.text}
        </span>
      </button>
    );
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center gap-3">
        ...
        <span className="flex-1 font-medium text-[#0f172a] dark:text-gray-200 flex items-center gap-1.5">
          {selected && (
            <span className="h-4 w-4 rounded-full bg-primary-500 flex items-center justify-center flex-shrink-0">
        ...
        <span className="text-xs font-semibold text-[#64748b] tabular-nums">
          {option.percent}%{" "}
          <span className="font-normal text-[#94a3b8]">({option.count})</span>
        </span>
      </div>
    </div>
  );
      ...
      <div className="space-y-1.5">
        {[...results].reverse().map((r) => (
          <div key={r.star} className="flex items-center gap-2 text-sm">
            <span className="w-10 text-[#64748b] flex items-center gap-0.5">
              {r.star}
            ...
            <span className="w-8 text-right text-xs text-[#94a3b8] tabular-nums">
              {r.count}
            </span>
          </div>
        ))}
        {total > 0 && (
          <p className="text-xs text-[#94a3b8] pt-1">
            Average:{" "}
            {(
              results.reduce((s, r) => s + r.star * r.count, 0) / total
            ).toFixed(1)}{" "}
            / 5
          </p>
        )}
...
function OpenResults({ results, compact }) {
  if (!results?.length) {
    return <p className="text-sm text-[#94a3b8] italic">No answers yet.</p>;
  }
  return (
    <div
      className={clsx(
        "space-y-2 max-h-56 overflow-y-auto scrollbar-thin pr-1",
        compact && "max-h-32"
      )}
    >
      {results.map((r, i) => (
